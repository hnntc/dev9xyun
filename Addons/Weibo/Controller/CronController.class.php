...
class CronController extends BaseController{
	...
	public function action(){
		...
		if($this->access()){
			...
			//echo $public_token;
			//获取微博历史列表
			$weibo_history=M('weibo_history')->where(array('show'=>1))->select();
			$weibo_num=M('weibo_history')->where(array('show'=>1))->count();
			//dump($weibo_history);
			for($i=0;$i<$weibo_num;$i++){
				//当前微博的信息
				$weibo=$weibo_history[$i];
				$wid=$weibo['wid'];
				$weibo_message=json_decode($weibo['data'],true);
				$success_num=$weibo_message['success_num'];
				$success_array=$weibo_message['success_array'];
				...
				echo "comments:".$comments_count."  reposts:".$reposts_count."  attitudes:".$attitudes_count."<br>";
				$report=M('weibo_report')->where(array('wid'=>$wid))->find();
				//准备数据
				$data['comments']=$comments_count;
				$data['reposts']=$reposts_count;
				$data['attitudes']=$attitudes_count;
				$data['last_update_time']=date('Y-m-d H:i:s');
				if(!$report || $report==null){
					//没有数据 添加数据
					$data['wid']=$wid;
					$result=M('weibo_report')->add($data);
					$action="插入";
				}else{
					//已经有数据 进行更新
					$result=M('weibo_report')->where(array('cid'=>$report['cid']))->save($data);
					$action="更新";
				}
				if($result){
					echo "数据".$action."成功！<hr>";
				}
			}
		}else{
			//不是本地计划任务在执行  不允许访问
