...
function _initialize() {
	...
	public function lists(){
		//获取链接
		$addons=M('addons')->field('config')->where(array('name'=>'Weibo'))->find();
		$config=json_decode($addons['config'],true);
		define( "WB_AKEY" , $config['appkey'] );
		define( "WB_SKEY" , $config['secret'] );
		$hostname=$config['hostname'];
		$backurl=addons_url('Weibo://Grant/result');
		//$backurl=$hostname.'/weiphp/index.php?s=/addon/Weibo/Grant/result.html';
		define( "WB_CALLBACK_URL" ,$backurl);
		$o = new SaeTOAuthV2( WB_AKEY , WB_SKEY );
		$code_url = $o->getAuthorizeURL( WB_CALLBACK_URL );	
		$code_url.="&ownerid=".$this->member_uid();
		...
		public function grant(){
			...
			$this->error( "帐户不能为空");
				...
				$uid_get = $c->get_uid();
				$uid1 = $uid_get['uid'];
				$user_message = $c->show_user_by_id($uid1);//根据ID获取用户等基本信息
				$screen_name=$user_message['screen_name'];//用户名
				//查询并更新
				$access_list=M('access_list');
				$ownerid=$this->member_uid();
				$result=$access_list->where(array('uid'=>$uid,'ownerid'=>$ownerid))->find();
				//dump($access_list);
				$data=array();
				$data['uid']=$uid;
				...
				if(!$result){
					//有数据进行更新
					$return=$access_list->add($data);
					
					
				}else{
					$return=$access_list->where(array('ownerid'=>$ownerid,'uid'=>$uid))->save($data);
				}
				if($return){
					
					//$back="/onthink/index.php/home/ForHook";
					
					//$back_url='授权/更新完成<a href="'.$back.'"  >点击跳回操作界面</a>';
					
					//$this->assign("back_url",$back_url);
					$this->success(" 授权/更新完成",addons_url('Weibo://Weibo/lists'));
				}
			}
		}
		public function update_grant(){
			...
			$aid=$_REQUEST['aid'];
			$access_list=M('access_list')->where(array('aid'=>$aid,'ownerid'=>$ownerid))->find();
			if($access_list){
				$old_time=strtotime($access_list['get_time']);
				$now=time();
				$hour=($now-$old_time)/60;
				//更新频率不高于30分钟/次
				if($hour<30){
					$this->error('更新过频！');
				}else{
					...
					if($token){
						if($token['error'] != null){
							if($token['error_code']=="21326"){
								$this->error("本功能未开放使用！请谅解！");
							}else{
								$this->error("未知错误".$token['error']);
							}
							
						}else{
							$access_token=$token['access_token'];
							$date=date('Y-m-d H:i:s');
							$data=array('access_token'=>$access_token,'get_time'=>$date);

							$access=M('access_list')->where(array('aid'=>$aid))->save($data);
							//$this->success($str."更新成功".$token['access_token']);
							if($access){
								//$_SESSION['grant_ok']="ok";
								$this->success("授权更新成功！");
	...
	public function result(){
		//dump($_REQUEST);
				...
				define( "WB_SKEY" , $config['secret'] );
				$hostname=$config['hostname'];
				$back_url=$hostname.'/weiphp/index.php?s=/addon/Weibo/Grant/result.html';
				$success_url=$hostname.'/weiphp/index.php?s=/addon/Weibo/Weibo/lists.html';
				$o = new SaeTOAuthV2( WB_AKEY , WB_SKEY );
				/*if($_SESSION['grant_ok'] == "ok" && $_SESSION['code_result']!=$_REQUEST['code']){
					$message='授权更新成功(s)<a href="'.$success_url.'">点这里！</a>';
					$changename="no";
					$_SESSION['grant_ok'] = "notok";
				}else{
					$_SESSION['grant_ok'] = "notok";
					*/
				if (isset($_REQUEST['code'])) {
					...
					if($token){
						...
						echo "<br>uid1=".$uid1."<br>";
						echo "<br>uid=".$uid."<br>";
						*/
						$access=M('access_list');
						$testuid=$access->where(array('uid'=>$uid))->select();
						...
						$data['get_time']=date('Y-m-d H:i:s');
						$data['expires_in']=$expires_in;
						//echo "<br>testuid=".$testuid."--".count($testuid)."<br>";
						if($testuid){
							$changename="no";
							$used_ownerid=trim($testuid[0]['ownerid']);
							$thisuser=trim($this->member_uid());
							if($used_ownerid == $thisuser){
								//当前帐号已经使用过 这次操作为更新token操作
								$access->where(array('uid'=>$uid))->save($data);
								$message='授权更新成功<a href="'.$success_url.'">点这里！</a>';
								$error="update";
								//$_SESSION['grant_ok']="ok";
								//$_SESSION['code_result']=$_REQUEST['code'];
								
							}else{
								$message="该帐号已经被别的用户使用";
								//为该用户更新一下token
								$access->where(array('uid'=>$uid))->save($data);
								$error="used";
							}
							...
							if($screen_name== "" ||$screen_name == null){
								//登陆帐号不是新浪邮箱 必须进行更名
								$message="这个帐号注册时不是用的新浪邮箱，无法获取昵称信息，请给它起个别名吧！";
								$changename="yes";
								$data['screen_name']=$uid;
								$data['ownerid']=$this->member_uid();
								$aid_result=$access->add($data);
								
							}else{
								$thisuser_used=$access->where(array('screen_name'=>$screen_name,'ownerid'=>$this->member_uid()))->find();
								if($thisuser_used == null || count($thisuser_used)==0){
									//当前用户没有这个名字的授权
									$data['screen_name']=$screen_name;
									$data['ownerid']=$this->member_uid();
									$access->add($data);
									//添加数据成功
									$message='帐号授权添加完成！去列表看看把<a href="'.$success_url.'">点这里！</a>';
									//$_SESSION['grant_ok']="ok";
									//$_SESSION['code_result']=$_REQUEST['code'];
								}else{
									//当前用户有这个名字的授权
									$message="您已经用过这个昵称，换一个名字吧！";
									//显示更名框
									$changename="yes";
									$data['screen_name']=$uid;
									$data['ownerid']=$this->member_uid();
					...
					$changename="no";
					$message="授权已取消/或完成";
					//$url="http://www.weibodiy.com/weiphp/index.php?s=/addon/Weibo/Weibo/lists.html";
					//$this->redirect($url);
				}
		//}
		$this->assign('uid',$uid);
		$this->assign('changename',$changename);
	...
	public function updatename(){
		...
		$uid=$_REQUEST['uid'];
		$access=M('access_list');
		$find=$access->where(array('ownerid'=>$this->member_uid(),'screen_name'=>$screen_name))->find();
		if($find){
			$this->error("昵称占用，换一个吧");
		}else{
			$data=array();
			$data['screen_name']=$screen_name;
			$re=$access->where(array('uid'=>$uid))->save($data);
			$changename="no";
			$this->success("更改昵称成功",addons_url('Weibo://Weibo/lists'));
		}
...
class SaeTOAuthV2 {
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			$token['error']= "get access token failed." . $token['error'];
		}
		return $token;
	}
