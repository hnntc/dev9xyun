...
function _initialize() {
	...
	public function grant(){
		echo "saddddddddddssaaaaaaaaaaaaaaaaa";
	}
	public function lists(){		
		...
		$result = M ( 'weibo_history' )->where ( $map )->order ( 'wid DESC' )->page ( $page, $row )->select ();
		$count = M ( 'weibo_history' )->where ( $map )->count ();
		if ($count > $row) {
		...
		while(list($key,$val)= each($result)) {
			...
			$wid=$val['wid'];/*1*/
			$date=$val['date'];/*1*/
			$num=$data['success_num'];/*1*/
			if($num !=0){
				$success_array=$data['success_array'];
				$text=$success_array[0]['text'];/*1*/
				$temp=array('wid'=>$wid,'date'=>$date,'num'=>$num,'content'=>$text);
	...
	public function show(){
		...
		$public_token=$config['access_token'];
		$c = new SaeTClientV2( WB_AKEY , WB_SKEY , $public_token);
		$result=M('weibo_history')->where(array('wid'=>$wid,'ownerid'=>$this->member_uid()))->find();
		$wid=$result['wid'];/*1*/
		$date=$result['date'];/*1*/
		$data=json_decode($result['data'],true);
		$success_num=$data['success_num'];/*1*/
		$error_num=$data['error_num'];/*1*/
		$success_array=$data['success_array'];
		$text=$success_array[0]['text'];/*1*/
		$img_json=$result['pic_urls'];
		$img_array=json_decode($img_json,true);
		
		$info_array=array();/*1*/
		for($i=0;$i<$success_num;$i++){
			$uid=$success_array[$i]['uid'];/*2*/
			//获取accessToken
			$access_list=M('access_list');
			$access=$access_list->where(array('uid'=>$uid,'ownerid'=>$this->member_uid()))->find();
			
			if($access==null || count($access)==0){
				// "this is a key not used<br>";
				continue;
			}else{
				$public_token=$config['access_token'];
				$mid=$success_array[$i]['mid'];
				$screen_name=$success_array[$i]['screen_name'];
				
				$re=$access_list->where(array('uid'=>$uid,'ownerid'=>$ownerid))->find();
				if($re){
					$screen_name=$re['screen_name']; /*2*/
				}
				
			
				$ret = $c->show_status($mid);	//微博
				$comments_count=$ret['comments_count']==null?0:$ret['comments_count'];/*2*/
				$reposts_count=$ret['reposts_count']==null?0:$ret['reposts_count'];/*2*/
				$attitudes_count=$ret['attitudes_count']==null?0:$ret['attitudes_count'];/*2*/
				$temp=array(
					'uid'=>$uid,
					'mid'=>$mid,
		...
		$this->assign('error_num',$error_num);
		$this->assign ( 'list_data', $info_array);
		$this->display();
	}
	public function del(){
		$ids = I ( 'id', 0 );
		...
		if ($ids[0]==0 || !isset($ids)) {
			...
			//现在ids已经完成了数组序列化
			$history=M('weibo_history');
			$result=$history->where($map)->delete();
			//级联删除排行榜上的数据
			
			$jilian=M('weibo_report')->where($map)->select();
			if(count($jilian)!=0){
				$re=M('weibo_report')->where($map)->delete();
			}
			if($result){
				$this->success ( '删除成功');
			}else{
