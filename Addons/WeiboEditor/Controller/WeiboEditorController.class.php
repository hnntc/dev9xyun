...
namespace Addons\WeiboEditor\Controller;
use Home\Controller\AddonsController;

class WeiboEditorController extends AddonsController{
	...
	public function xiuxiuupload()
    {
        	if (!empty($_FILES)) {
           		...
           		$config = array(   
            		...
            		$upload = new \Think\Uploadforweibo($config);// 实例化上传类
					$images = $upload->upload();
					//判断是否有图
					if($images){
					
               		 $info=$images['upload_file']['savepath'].$images['upload_file']['savename'];
               		 //返回文件地址和名给JS作回调用
	...
	public function send(){
		//dump($_POST);
		$pic_array=$_REQUEST['pic_urls'];//图片路径s更改接口时使用！仅有图片的日期目录个图片名称
		/////www.weibodiy.com/  __ROOT__/Uploads/ $pic_array[$i]
		$pic_urls=json_encode($pic_array);
		if($_POST['text_info'] == null ||$_POST['text_info'] == '' ||ctype_space($_POST['text_info'])){
			...
			if($ownerid == '' ||$ownerid==null){
				...
				$available=M('available_token');
				$tre=$available->where(array('ownerid'=>$ownerid))->find();
				$info=json_decode($tre['available_list'],true);
			
				if(count($info['aids'])==0){
					$data['status']= "notoken";
					$json_data=json_encode($data);
				}else{
					$data['status']="ok";
					...
					$error_array=array();
					$success_array=array();
					for($i=0;$i<count($info['aids']);$i++){
						$aid=$info['aids'][$i];
						$access_list=M('access_list');
						$access=$access_list->where(array('aid'=>$aid))->find();
						$access_token=$access['access_token'];
						$uid=$access['uid'];
						$screen_name=$access['screen_name'];
						//成功拿到钥匙！！！
						//echo $uid.'-'.$access_token."<br>";
						$c = new SaeTClientV2( WB_AKEY , WB_SKEY , $access_token);
						//判断是否有图片
						if(count($pic_array)==0){
							//纯文本微博
							$ret = $c->update( $_REQUEST['text_info'] );	//发送微博
						}else if(count($pic_array)==1){
							//含图片微博
							//$img_path="www.weibodiy.com/weiphp/Uploads".$pic_array[0];
							//一定要记得修改
							
							$img_path=$hostname."/Uploads/".$pic_array[0];
							$ret = $c->upload( $_REQUEST['text_info'] ,$img_path);	//发送微博
						}else if(count($pic_array) >1){
								//$status=$_REQUEST['text_info'];
								/*
								$status="haha有很多图片哦！";
								$urls="";
								$pic_ids="";
								for($i=0;$i<count($pic_array);$i++){
									$pic_array[$i]=$hostname."/weiphp/Uploads/".$pic_array[$i];
									//$pic_id=$c->upload_pic($access_token,$pic_array[$i]);
									$pic_ids.=$pic_id.",";
								}
								$pic_url=implode(',',$pic_array);
								//
								$ret = $c->upload_url_text($status,$pic_url);
								//$status,$url, $visible=0, $list_id=NULL, $pic_id=NULL, $lat = NULL, $long=NULL,
								*/
						}
					...
					//汇总信息
						...
						//构造返回数据
						$json_data=json_encode($data);
						$send_history=M('weibo_history');
						$add['pic_urls']=$pic_urls;
						$add['ownerid']=$ownerid;
						$add['data']=$json_data;
...
class SaeTClientV2
{
	 ...
	 * @return array
	 */
	function upload( $status, $pic_path, $lat = 50, $long = -20 )
	{
		$params = array();
		$params['status'] = $status;
		$params['pic'] = '@'.$pic_path;
	...
	//diy-add//
	function upload_pic($access_token,$url){
		$params = array();
		...
		return $this->oauth->post( 'statuses/upload_pic', $params, true );
	}
